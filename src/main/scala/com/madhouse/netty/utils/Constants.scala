...
//  val config = ConfigFactory.load("application_dev")
  ...
  val brokers = getOrElse("kafka.brokers", "localhost:9092").toString
  val topic = getOrElse("kafka.topic", "mklog").toString

  val engine_port = config.getInt("engine.port")
  val engine_compact = config.getBoolean("engine.isCompact")

  val engine_compact_symbol = config.getString("engine.compactSymbol")
  val interval_send = config.getInt("interval.send")
  val interval_sync = config.getInt("interval.sync")
